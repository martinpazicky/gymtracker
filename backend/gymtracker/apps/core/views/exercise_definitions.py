...
from django.db.models import  OuterRef, Subquery, F, IntegerField, FloatField
...
class ExerciseDefinitionsViewSet(viewsets.ModelViewSet):
    ...
    def personal_records(self, request):
        ...
        # approach without calling subqueries multiple times, but the queries look even more inefficient
        from django.db import connection, reset_queries
        ...
        for i, row in enumerate(values):
            heaviest_set = heaviest_sets_x[i]._prefetched_objects_cache['exerciserealization_set'][0].heaviest_seat[0]
        ...
        # print(heaviest_setsa.values(x=F('heaviest_seat')))
        # print(heaviest_sets.heaviest_sets)
        return Response(connection.queries)

        
        # 2.approach using raw SQL postgres and postgres json functions (json forming could also be done in memory (dictfetchall) or via serializers)
